...
def create_showcase_images(seed: int = 0) -> list[np.ndarray]:
    ...
    gcamp = snap_channel(sim, mode=1, exposure=60.0)
    img2 = apply_fluorescence(gcamp, GREEN)

    for _ in range(5):
        sim.step(dt=1.0)
    gcamp2 = snap_channel(sim, mode=1, exposure=60.0)
    img3 = apply_fluorescence(gcamp2, GREEN)

    bf2 = snap_channel(sim, mode=0, exposure=50.0)
    img4 = composite_max(apply_cmap(bf2, "gray"), apply_fluorescence(gcamp2, GREEN))
