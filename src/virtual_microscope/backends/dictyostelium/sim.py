...
class DictyosteliumSim:
    ...
    def _update_mode(self):
        ...
        filt = self.state_devices.get("Filter Wheel", {})
        led_label = led.get("label", led.get("Label", ""))
        filt_label = filt.get("label", filt.get("Label", ""))

        if "CYAN" in led_label or "Electra" in filt_label:
            self.mode = 0
        elif "ORANGE" in led_label or "mScarlet" in filt_label:
            self.mode = 1
        elif "RED" in led_label or "miRFP" in filt_label:
            self.mode = 2
        else:
            self.mode = 0
