...
class MicrofluidicsSim:
    ...
    def _update_mode(self):
        if "Filter Wheel" not in self.state_devices or "LED" not in self.state_devices:
            self.mode = 0
            return
        fl = self.state_devices["Filter Wheel"].get("label",
             self.state_devices["Filter Wheel"].get("Label", ""))
        ll = self.state_devices["LED"].get("label",
             self.state_devices["LED"].get("Label", ""))
