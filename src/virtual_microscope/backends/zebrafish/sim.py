...
class ZebrafishSim:
    ...
    def _update_mode(self):
        if ("Filter Wheel" not in self.state_devices
                or "LED" not in self.state_devices):
            self.mode = 0
            return
        ...
        if "TAGGFP2" in combined or "GREEN" in combined:
            self.mode = 1  # vascular GFP
        elif "MSCARLET3" in combined or "ORANGE" in combined:
            self.mode = 2  # cardiac mCherry
        elif "MIRFP670" in combined or "RED" in combined:
            self.mode = 2
        else:
